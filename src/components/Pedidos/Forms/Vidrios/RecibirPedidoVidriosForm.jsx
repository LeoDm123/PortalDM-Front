...
const RecibirPedidoVidriosForm = ({
  onClose,
  pedidoId,
  codigoMat,
  onSubmit,
  ...
  const fetchPedidoData = async () => {
    try {
      const resp = await serverAPI.get(
        `/pedidoVidrios/obtenerPedidoPorId/${pedidoId}`
      );
  ...
  const handleSubmit = async (event) => {
    ...
    const Descripcion = "Vidrio: " + Ancho + " x " + Alto + " - " + Composicion;

    try {
      await serverAPI.put(
        `/pedidoVidrios/recibirPedido/${pedidoId}/${codigoMat}`,
        {
          CantRecibida,
          FechaRecep,
          nroPedido,
...
export default RecibirPedidoVidriosForm;
